...
class MoleculeDataset_Invariance_V2(Dataset):
    def __init__(self, Data_List, Nb_Atoms_Max_In_Molecule=23, Return_Energies=True, transform=None):
        """
        Dataset pour des molécules avec positions représentées par [distance_CM, angle_CMA, angle_CMB] en 3D.
    ...
    def __getitem__(self, idx):
        ...
        new_positions = np.zeros((n_atoms, 3), dtype=np.float32)
        vec_CA = pos_A - center
        vec_CB = pos_B - center
        ...
        for i in range(n_atoms):
            ...
            # Angle CMA (angle entre CM et CA, sans signe)
            ...
            # Angle CMB (angle entre CM et CB, sans signe)
...
def Create_DataLoader_Invariance_V2(Data_List, Batch_Size=32, Test_Size=0.3, Shuffle=True, Num_Workers=0,
    ...
    """
    dataset = MoleculeDataset_Invariance_V2 (
        Data_List,
